...
# Debug APK 빌드 및 설치 스크립트
# 사용: ./scripts/build-and-install-debug.sh
...
if [ -f "$PROJECT_DIR/.env.development" ]; then
    echo "✅ .env.development 파일 발견"
    echo "   NOTION_API_TOKEN: $(grep NOTION_API_TOKEN $PROJECT_DIR/.env.development | cut -d'=' -f2 | head -c 20)..."
else
    echo "⚠️  .env.development 파일이 없습니다"
    exit 1
fi
...
echo ""
echo "Logcat으로 로그 확인:"
echo "  adb logcat | grep -E 'IntegratedListener|KakaoTalk|SMS|S3Service'"
echo ""
echo "=========================================="
